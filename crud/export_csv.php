...
$full_query = "SELECT gate_number, building, location, DAYNAME(timestamp) as day_of_week, timestamp AS hour_block, original_timestamp FROM $default_table_name WHERE DATE(timestamp) >= '$date_start' AND DATE(timestamp) <= '$date_end' AND $location_where ORDER BY hour_block DESC";
